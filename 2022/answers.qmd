...
read_tsv(col_names = "header", file = "misc/solved_days", 
         show_col_types = FALSE) %>%
  # create the hyperlink to the contents
  mutate(day = str_extract(header, "[0-9]+"), 
         day = as.numeric(day),
         Day = glue::glue("[{day}]({header})")) %>%
  # add the stars
  full_join(stars_df) %>%
  # fill with days not solved
  full_join(empty_days_df) %>%
