...
readr::read_tsv(col_names = "header", file = "misc/solved_days", 
  ...
  dplyr::mutate(day = case_when(
    !is.na(day) ~ day,
    !is.na(dplyr::lag(day)) ~ dplyr::lag(day),
    !is.na(dplyr::lag(day, 2)) ~ dplyr::lag(day, 2)
  )) %>%
  group_by(day) %>%
  mutate(`Part 1` = case_when(
    any(stringr::str_detect(header, "##part-one-⭐" )) ~ "⭐",
    TRUE ~ ""
  ), 
  `Part 2` = case_when(
    any(stringr::str_detect(header, "##part-two-⭐" )) ~ "⭐",
    TRUE ~ ""
  )) %>% 
  ungroup() %>%
  filter(stringr::str_detect(header, "#day")) %>%
...
[Description of the task](https://adventofcode.com/2022/day/10#part2)
...
[Full task outline](https://adventofcode.com/2022/day/18).
...
[Part two of the problem](https://adventofcode.com/2022/day/18#part2).

I'm having 

```python
def build_walls(file_path):
  with open(file_path, "r+") as f:
    lines = f.readlines()
...
```python
a = build_walls("inputs/18_test_case.txt")
walls = np.zeros(a.shape)
walls[np.isinf(a)] = -1 
flood = np.zeros(walls.shape)
def walk_along(walls, flood, pos):
  pos_t = tuple(pos)
  ...
  for c in [-1, +1]: 
    for i in range(len(arr.shape)):
      pos_ = pos.copy()
      pos_[i] += c
      print(f"c: {c} in {i} position of {pos} resulting in {pos_}")
      print(max(flood.shape))
      if (max(pos_) <= max(flood.shape)) & (min(pos_) > 0):
        flood += walk_along(walls, flood, pos_)
      
  return flood

walk_along(c, np.zeros(c.shape), [0, 0])
```


```python
count_walls_outside("inputs/18_input.txt")
```
