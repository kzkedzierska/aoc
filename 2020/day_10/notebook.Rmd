...
```{r}
get_essential_elements <- function(vec, n = 3, add_to_device = 3) {
  stopifnot(is.numeric(vec))
  border_vec <- c(0, max(vec) + add_to_device)
  vec <- c(border_vec, vec)
  sorted_vec <- sort(vec)
  have_to_stay <- 
    c(sorted_vec[which(sorted_vec - lag(sorted_vec, 1) == 3) - 1],
      sorted_vec[which(sorted_vec - lag(sorted_vec, 1) == 3)])

  print(sort(have_to_stay))  
...
calc_number_of_possible_solutions <- function(vec) {
  stopifnot(is.numeric(vec))
  essential_vec <- get_essential_elements(vec)
  n <- length(setdiff(vec, essential_vec))
  2^n
}

calc_number_of_possible_solutions(demo_input_vec) == 8
```

```{r}
calc_number_of_possible_solutions(demo_input_vec2)
```



```{r}
sorted_demo_vec2 <- sort(c(0, 52, demo_input_vec2))
...
```{r}
f(1, c(0, sort(demo_input_vec2), max(demo_input_vec2)+3)) == 19208
```
